...
// Copy of TestFixedBitSet with extra tests specific for FixedBitSet
public class TestTrackedFixedBitSet extends BaseBitSetTestCase<FixedBitSet> {

  @Override
  public FixedBitSet copyOf(org.apache.lucene.util.BitSet bs, int length) throws IOException {
  ...
  public void testFillTrackersTiny() {
    ...
    bitset.set(0);
    FixedBitSet tracked = new FixedBitSet(bitset.bits, bitset.numBits);
  ...
  public void testFillTrackersSmall() {
    FixedBitSet bitset = new FixedBitSet(256); // 4 longs
    bitset.set(0);
    bitset.set(255);
    FixedBitSet tracked = new FixedBitSet(bitset.bits, bitset.numBits);
  ...
  public void testFillTrackersMedium() {
    FixedBitSet bitset = new FixedBitSet(64*64+1); // 65 longs (2 entries in tracker 1)
    bitset.set(0);
    bitset.set(255);
  ...
  public void testFillTrackersLarge() {
    FixedBitSet bitset = new FixedBitSet(64*64*64+1); // 64*64+1 longs (2 entries in tracker 2)
    bitset.set(255);
    bitset.set(4096);
  ...
  /**
   * Verify that {@link org.apache.lucene.util.FixedBitSet#set(int)} and
   * {@link org.apache.lucene.util.FixedBitSet#getAndSet(int)} updates the trackers correctly.
   */
  ...
  public void testTrackedSetRange128() {
    FixedBitSet bitset = new FixedBitSet(256);
    bitset.set(1, 128);
  ...
  private interface TrackedDualCallback {
    public void update(FixedBitSet bitset, Integer start, Integer end);
  }
  ...
  private void testTrackedUpdates(String message, int runs, int maxSize, int maxUpdates, TrackedDualCallback callback) {
    for (int r = 0 ; r < runs ; r++) {
      FixedBitSet bitset = getRandomTracked(message + ". Monkey run=" + r, 1+maxSize, maxUpdates);
  ...
  public void testTrackedSetRangeMonkey() {
    ...
    // Java 1.8 code: testTrackedUpdates("set range", FixedBitSet::set);
  }
  ...
  public void testTrackedClearRangeMonkey() {
    ...
    // Java 1.8 code: testTrackedUpdates("clear range", FixedBitSet::clear);
  }
...
// Java 1.8 code: testTrackedUpdates("flip range", FixedBitSet::flip);
  }
  ...
  public void testIntersects() {
    ...
    for (int r = 0 ; r < RUNS ; r++) {
      FixedBitSet bitset1 = getRandomTracked(10000, 1000);
      FixedBitSet bitset2 = getRandomTracked(10000, 1000);
  ...
  public void testTrackedBitAdvance() throws IOException {
    ...
    FixedBitSet bitset = new FixedBitSet(64*64*10);
    for (int bit: BITS) {
      bitset.set(bit);
    }
...
/*    { // Base check
  ...
  */
  ...
  public void testTrackedWordAdvance() {
    ...
    { // Base check
      final int[] WORDNUMS = new int[]{0, 1, 63, 64, 80};
      FixedBitSet.WordIterator words = bitset.wordIterator();
    ...
    { // Advance B
      ...
      assertEquals("B. Nor more docs", FixedBitSet.WordIterator.NO_MORE_DOCS, words.nextWordNum());
    }
    { // Advance B
      FixedBitSet.WordIterator words = bitset.wordIterator();
      assertEquals("B. Advance start", 0, words.nextWordNum());
    ...
    { // Advance C
      ...
      assertEquals("C. Advance 129 (skip over end)",
          FixedBitSet.WordIterator.NO_MORE_DOCS, words.advance(129));
    }
  }
  ...
  private void testTrackedWordIterator(FixedBitSet bitset) {
    ...
    }
    {
      ...
      int lastWordNum = -1;
      FixedBitSet.WordIterator wi = new FixedBitSet.WordIterator(bitset);
      while (wi.nextWordNum() != FixedBitSet.WordIterator.NO_MORE_DOCS) {
        wordIteratorCount++;
  ...
  public void testTrackedIntersectionCount() {
    FixedBitSet bitset1 = getRandomTracked("Intersection", 10000, 100); // 1% load
    FixedBitSet bitset2 = getRandomTracked("Intersection", 10000, 100); // 1% load

    long expected = BitUtil.pop_intersect(bitset1.bits, bitset2.bits, 0, Math.min(bitset1.numWords, bitset2.numWords));
    long actual = FixedBitSet.intersectionCount(bitset1, bitset2);
    assertEquals("Tracked intersection count should match non-tracked", expected, actual);
  }

  public void testTrackedUnionCount() {
    FixedBitSet a = getRandomTracked("Intersection", 10000, 100); // 1% load
    FixedBitSet b = getRandomTracked("Intersection", 10000, 100); // 1% load
  ...
  public void testMerge() { // Triggered by missing word 120
    ...
    Random random = new Random(SEED);
    FixedBitSet bitset1 = getRandomTrackedFixed(random, 64*139, 25);
    FixedBitSet bitset2 = getRandomTrackedFixed(random, 64*123, 25);
    long nonZeroTracked = FixedBitSet.merge(bitset1, bitset2, false,
         new FixedBitSet.MergeCallback() {
           @Override
  ...
  public void testTrackedXor(String message, Random random, int size1, int size2, int updates1, int updates2) {
    //FixedBitSet bitset1 = getRandomTrackedFixed(random, size1, updates1);
    //FixedBitSet bitset2 = getRandomTrackedFixed(random, size2, updates2);
    FixedBitSet bitset1 = getRandomTracked(10000, 100);
    FixedBitSet bitset2 = getRandomTracked(10000, 100);
    if (bitset2.numBits > bitset1.numBits) {
      FixedBitSet temp = bitset1;
      bitset1 = bitset2;
      bitset2 = temp;
    }

    FixedBitSet tracked1 = bitset1.clone();
    FixedBitSet tracked2 = bitset2.clone();

    FixedBitSet clone1 = bitset1.clone();
    FixedBitSet clone2 = bitset2.clone();
  ...
  private void testAnd(String message, Random random, int size1, int size2, int updates1, int updates2) {
    FixedBitSet bitset1 = getRandomTracked(size1, updates1);
    FixedBitSet bitset2 = getRandomTracked(size2, updates2);
...
//    }
  ...
  private FixedBitSet getRandomTracked(int maxSize, int maxUpdates) {
    return getRandomTracked("Sample request", maxSize, maxUpdates);
  }
  private FixedBitSet getRandomTracked(String message, int maxSize, int maxUpdates) {
    int updates = random().nextInt(maxUpdates /4);
    FixedBitSet bitset = getRandomTrackedFixed(random().nextInt(maxSize -1)+1, updates);
    assertTrackers(message + ": size=" + bitset.numBits + ", updates=" + updates, bitset);
    return bitset;
  }
  private FixedBitSet getRandomTrackedFixed(int size, int updates) {
    return getRandomTrackedFixed(random(), size, updates);
  }
  private FixedBitSet getRandomTrackedFixed(Random random, int size, int updates) {
  ...
  private void testTrackedNextSetBit(String message, FixedBitSet bitset) throws IOException {
    DocIdSetIterator bits = bitset.iterator();
    int iPos;
    int nPos = 0;
  ...
  public void testTrackedPrevSetBitA() throws IOException {
    FixedBitSet bitset = new FixedBitSet(100000);
    bitset.set(7);
    bitset.set(87);
    bitset.set(640);
    bitset.set(7001);
  ...
  public void testTrackedPrevSetBitB() throws IOException {
    FixedBitSet bitset = new FixedBitSet(100000);
    bitset.set(16384);
    bitset.set(65537);
    testTrackedPrevSetBit("PrevSetBit specific", bitset);
  }
  private void testTrackedPrevSetBit(String message, FixedBitSet bitset) throws IOException {
    DocIdSetIterator bits = bitset.iterator();
  ...
  public void testTrackedCardinalityMonkey() {
    ...
    for (int r = 0 ; r < RUNS ; r++) {
      FixedBitSet tracked = getRandomTracked("Monkey run=" + r, MAX_SIZE, MAX_UPDATES);
  ...
  private void assertTrackers(String message, FixedBitSet tracked) {
    FixedBitSet recalculated = new FixedBitSet(tracked.bits, tracked.numBits);
