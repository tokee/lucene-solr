...
//    }
    return GenerateLongtailDistribution(histogram, seed);
  }

  public static int[] GenerateLongtailDistribution(int[] histogram, long seed){
    int totalSize = 0;
    for (int v: histogram) {
      totalSize += v;
    }
...
//    System.out.println("array size:"+totalSize +" prime1:"+BIJECTIONPRIME1 +" prime2:"+BIJECTIONPRIME2);
    ...
    for (int i=0;i<histogram.length;i++){
      ...
      for (int j=0;j<histogram[i];j++){
        distribution[getBijectionMapping(index++,distribution.length)]=localMax;
