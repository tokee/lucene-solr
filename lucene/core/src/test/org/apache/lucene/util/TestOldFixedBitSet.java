...
public class TestOldFixedBitSet extends BaseBitSetTestCase<OldFixedBitSet> {

  @Override
  public OldFixedBitSet copyOf(BitSet bs, int length) throws IOException {
    final OldFixedBitSet set = new OldFixedBitSet(length);
  ...
  void doGet(java.util.BitSet a, OldFixedBitSet b) {
  ...
  void doNextSetBit(java.util.BitSet a, OldFixedBitSet b) {
    assertEquals(a.cardinality(), b.cardinality());
    int aa=-1;
    int bb=-1;
    do {
  ...
  void doPrevSetBit(java.util.BitSet a, OldFixedBitSet b) {
    assertEquals(a.cardinality(), b.cardinality());
  ...
  // test interleaving different OldFixedBitSetIterator.next()/skipTo()
  void doIterate(java.util.BitSet a, OldFixedBitSet b, int mode) throws IOException {
    if (mode==1) doIterate1(a, b);
    if (mode==2) doIterate2(a, b);
  }

  void doIterate1(java.util.BitSet a, OldFixedBitSet b) throws IOException {
    assertEquals(a.cardinality(), b.cardinality());
    int aa=-1,bb=-1;
  ...
  void doIterate2(java.util.BitSet a, OldFixedBitSet b) throws IOException {
    assertEquals(a.cardinality(), b.cardinality());
    int aa=-1,bb=-1;
    DocIdSetIterator iterator = BitSetIterator.getIterator(b, 0);
    do {
  ...
  void doRandomSets(int maxSize, int iter, int mode) throws IOException {
    ...
    for (int i=0; i<iter; i++) {
      ...
      OldFixedBitSet bb = b.clone(); bb.flip(fromIndex,toIndex);
      ...
      if (b0 != null && b0.length() <= b.length()) {
        ...
        OldFixedBitSet b_and = b.clone(); assertEquals(b,b_and); b_and.and(b0);
        OldFixedBitSet b_or = b.clone(); b_or.or(b0);
        OldFixedBitSet b_xor = b.clone(); b_xor.xor(b0);
        OldFixedBitSet b_andn = b.clone(); b_andn.andNot(b0);
  ...
  */
  ...
  public void testEquals() {
    ...
    OldFixedBitSet b1 = new OldFixedBitSet(numBits);
    OldFixedBitSet b2 = new OldFixedBitSet(numBits);
  ...
  private OldFixedBitSet makeOldFixedBitSet(int[] a, int numBits) {
    OldFixedBitSet bs;
    if (random().nextBoolean()) {
      int bits2words = OldFixedBitSet.bits2words(numBits);
      long[] words = new long[bits2words + random().nextInt(100)];
      bs = new OldFixedBitSet(words, numBits);
    } else {
      bs = new OldFixedBitSet(numBits);
    }
    for (int e: a) {
      bs.set(e);
    }
  ...
  private void checkPrevSetBitArray(int [] a, int numBits) {
    OldFixedBitSet obs = makeOldFixedBitSet(a, numBits);
  ...
  public void testEnsureCapacity() {
    OldFixedBitSet bits = new OldFixedBitSet(5);
    bits.set(1);
    bits.set(4);
    
    OldFixedBitSet newBits = OldFixedBitSet.ensureCapacity(bits, 8); // grow within the word
    ...
    newBits.set(1);
    newBits = OldFixedBitSet.ensureCapacity(newBits, newBits.length() - 2); // reuse
    assertTrue(newBits.get(1));

    bits.set(1);
    newBits = OldFixedBitSet.ensureCapacity(bits, 72); // grow beyond one word
    assertTrue(newBits.get(1));
    assertTrue(newBits.get(4));
    newBits.clear(1);
  ...
  public void testBits2Words() {
    assertEquals(0, OldFixedBitSet.bits2words(0));
    assertEquals(1, OldFixedBitSet.bits2words(1));
    // ...
    assertEquals(1, OldFixedBitSet.bits2words(64));
    assertEquals(2, OldFixedBitSet.bits2words(65));
    // ...
    assertEquals(2, OldFixedBitSet.bits2words(128));
    assertEquals(3, OldFixedBitSet.bits2words(129));
    // ...
    assertEquals(1024, OldFixedBitSet.bits2words(65536));
    assertEquals(1025, OldFixedBitSet.bits2words(65537));
    // ...
    assertEquals(1 << (31-6), OldFixedBitSet.bits2words(Integer.MAX_VALUE));
  }
  ...
  public void testIntersectionCount() {
    ...
    OldFixedBitSet fixedBitSet1 = makeOldFixedBitSet(bits1, numBits1);
    OldFixedBitSet fixedBitSet2 = makeOldFixedBitSet(bits2, numBits2);
    ...
    long intersectionCount = OldFixedBitSet.intersectionCount(fixedBitSet1, fixedBitSet2);
  ...
  public void testUnionCount() {
    ...
    long unionCount = OldFixedBitSet.unionCount(fixedBitSet1, fixedBitSet2);
  ...
  public void testAndNotCount() {
    ...
    long andNotCount = OldFixedBitSet.andNotCount(fixedBitSet1, fixedBitSet2);
